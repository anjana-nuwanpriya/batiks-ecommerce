@php
    ...
    // Clean cart of deleted products on every load
            ...
            @if(!empty(session()->get('cart')))
                <div class="cart-items">
                    @foreach(session()->get('cart') as $productId => $item)
                        @php
                            ...
                            // Skip if variant no longer exists
                            if (!$variant) {
                                continue;
                            }
                        @endphp
                        ...
                        <div class="d-flex position-relative mb-3 border-bottom pb-2" id="cart-item-{{ $product->id }}">
                            ...
                            <div class="flex-grow-1 ms-3">
                                ...
                                @if ($variant->variant != "Standard")
                                    <p class="text-muted font-12 mb-0">{{ $variant->variant }}</p>
                                @endif
                ...
                <div class="mt-auto border-top pt-3">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        @php
                            $totalAmount = CartHelper::getCartTotalAmount();
                        @endphp
                        <span class="fw-bold text-success" id="cart-total-amount">{{ formatCurrency($totalAmount) }}</span>
                    </div>
