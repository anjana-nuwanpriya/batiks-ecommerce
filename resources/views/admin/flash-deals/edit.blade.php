...
@section('title', 'Edit Flash Deal - '.env('APP_NAME'))
    ...
    <div class="d-flex justify-content-between align-items-center">
        ...
        <div>
            <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active">{{ __('Edit Flash Deal') }}</li>
            </ol>
        </div>
    </div>
@endsection
...
@section('content')
    <form action="{{ route('flash.deal.update', $flashDeal->id) }}" method="POST" class="ajax-form position-relative">
        @csrf
        @method('PUT')
        <div class="row pb-5">
            <div class="col-md-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="title" class="form-control" id="title" placeholder="{{ __('Title') }}" value="{{ $flashDeal->title }}">
                        ...
                        <div class="form-group">
                            ...
                            <x-adminlte-select2 name="products[]" id="products" multiple>
                                @foreach ($products as $product)
                                    <option value="{{ $product->id }}" {{ in_array($product->id, $flashDeal->items->pluck('product_id')->toArray()) ? 'selected' : '' }}>{{ $product->name }}</option>
                                @endforeach
                            </x-adminlte-select2>
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <x-adminlte-select2 name="status" id="status">
                                <option value="1" {{ $flashDeal->status ? 'selected' : '' }}>{{ __('Active') }}</option>
                                <option value="0" {{ !$flashDeal->status ? 'selected' : '' }}>{{ __('Inactive') }}</option>
                            </x-adminlte-select2>
                        </div>
                    </div>
                </div>
        ...
        <div class="btns-container">
            ...
            <a href="{{ route('flash.deal.index') }}" class="btn btn-secondary">{{ __('Cancel') }}</a>
        </div>
    </form>
@endsection
    ...
    <script>
        $(function() {
            $('#date_range').flatpickr({
                mode: 'range',
                dateFormat: 'Y-m-d',
                defaultDate: ['{{ $flashDeal->start_date }}', '{{ $flashDeal->end_date }}'],
                allowInput: true,
                locale: {
                    firstDayOfWeek: 1
                }
    ...
    <script>
        $(document).ready(function() {
            function renderSelectedProducts() {
                ...
                @php
                    $productsArr = [];
                    foreach($flashDeal->items as $item) {
                        $productsArr[$item->product_id] = [
                        ...
                        <div class="card mb-2 product-discount-row" data-product-id="${productId}">
                            <div class="card-body py-2">
                                <div class="row align-items-center">
                                    ...
                                    <div class="col-md-2">
                                        <input type="number" name="qty[${productId}]" class="form-control" min="1" step="1" placeholder="{{ __('Qty') }}" value="${productData.qty || ''}">
                                    </div>
                                    <div class="col-md-3">
                                        <input type="number" name="discount[${productId}]" class="form-control" step="0.01" min="0" placeholder="{{ __('Discount') }}" value="${productData.discount || ''}">
                                    </div>
                                    <div class="col-md-3">
                                        <select name="discount_type[${productId}]" class="form-control">
                                            <option value="percent" ${productData.discount_type === 'percent' ? 'selected' : ''}>{{ __('Percent') }}</option>
                                            <option value="fixed" ${productData.discount_type === 'fixed' ? 'selected' : ''}>{{ __('Fixed Amount') }}</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
